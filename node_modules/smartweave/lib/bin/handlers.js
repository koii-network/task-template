...
function readCommandHandler(argv) {
    ...
    The following is the current status of the contract ${chalk_1.default.bgBlack(chalk_1.default.white(contractId))}: 
    `);
            argv.prettify
...
function writeCommandHandler(argv) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const wallet = JSON.parse(fs_1.readFileSync(argv.keyFile).toString());
        const jsonInput = utils_2.getJsonInput(input);
        input = jsonInput || input;
        try {
            let result;
            if (dryRun) {
                result = yield Sdk.interactWriteDryRun(arweave, wallet, contractId, input);
                console.log(result);
            }
            else {
                result = yield Sdk.interactWrite(arweave, wallet, contractId, input);
                console.log(`Interaction posted at: ${result}`);
            }
        }
        catch (e) {
            loglevel_1.default.error(e);
            loglevel_1.default.error('Unable to excute write.');
        }
    });
}
